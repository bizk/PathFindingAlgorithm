...
public class Intento3Cordoba {
	...
	private void demoObtenerCamino() {
		...
		if (iter.hasNext()) {
			...
			if(!listaPuntos.isEmpty()) {
				...
				mapa.enviarMensaje("No hay camino que una los puntos");
		}
	}
	
	private List<Punto> expandirPuntosContiguos(Punto a, Punto b) {
		matriz = new NodoPunto[MapaInfo.LARGO][MapaInfo.ALTO];
						 ...
						 if(chequearPunto(pto, nodo)) {
							// aux = new NodoPunto(pto, nodo.getDensidad() + (mapa.getDensidad(pto)+1)*(esDiagonal(nodo.getP(),pto)? 14:10), nodo.getPuntos(), nodo);
							 ...
							 if(matriz[i][j] == null) {
								 ...
								 if(aux.getDensidad() < matriz[aux.getP().x][aux.getP().y].getDensidad()) {
									 matriz[aux.getP().x][aux.getP().y]=aux;
								 }
							 }
						 }
					 }
	...
	private List<Punto> getCamino(NodoPunto solucion) {
		...
		while (aux!=null) {
			camino.add(aux.getP());
			aux=matriz[aux.getP().x][aux.getP().y].getPredecesor();
		}
	...
	private boolean esDiagonal(Punto nodo, Punto punto) { //Chequea las 4 posibles diagonales.
		if((punto.x == nodo.x -1 && punto.y == nodo.y - 1)
				|| (punto.x == nodo.x + 1 || punto.y == nodo.y + 1)
				|| (punto.x ==nodo.x+1 && punto.y==nodo.y-1)
				|| (punto.x == nodo.x-1 && punto.y==nodo.y+1) )return true;
	...
	private boolean chequearPunto(Punto pto, NodoPunto nodo) {
		int x = pto.x, y = pto.y;
		if(x >= 0 && x < MapaInfo.LARGO && y >= 0 && y < MapaInfo.ALTO) {
